...
_active_shared_knowledge: str = ""  # GraphRAGから取得した共有知識（文書・統計）
_active_interpolated_context: str = ""  # LLM補間情報（競合・収益モデル・市場規模等）
...
class OASISSimulationEngine:
    ...
    async def run(self, num_rounds: int | None = None) -> list[RoundResult]:
        ...
        global _active_shared_knowledge, _active_interpolated_context
        ...
        # GraphRAGから共有知識（文書・統計）を1回取得してキャッシュ
        _active_shared_knowledge = await self._fetch_shared_knowledge()

        # LLM補間情報をエージェント共有コンテキストとして構築
        _active_interpolated_context = _build_interpolated_context(self._enriched_scenario)
        ...
        try:
            ...
            _active_agent_type_map = {}
            _active_action_history = []
            _active_shared_knowledge = ""
            _active_interpolated_context = ""

        return self.results

    async def _fetch_shared_knowledge(self) -> str:
        """GraphRAGから文書知識・統計データを取得する（シミュレーション開始時に1回）.
        ...
        try:
            ref_stats = await self._rag._get_reference_stats()
            if ref_stats:
                sections.append(ref_stats)
        except Exception:
    ...
    def _patch_oasis_japanese() -> None:
        ...
        async def _jp_perform_action(self: Any) -> Any:  # type: ignore[override]
            ...
            # LLM補間情報（競合・収益モデル・市場規模等）
            if _active_interpolated_context:
                market_context += f"\n{_active_interpolated_context}\n"
...
def _build_per_agent_contexts() -> dict[int, str]:
    ...
    加えて、GraphRAGから取得した共有知識（文書・統計）を全エージェントに付与する。
    """
    if not _active_action_history and not _active_shared_knowledge:
        return {}
    ...
    # GraphRAG共有知識（文書・統計）を全エージェントに付与
    if _active_shared_knowledge:
        for oasis_id in _active_agent_type_map:
            existing = contexts.get(oasis_id, "")
            if existing:
                contexts[oasis_id] = existing + "\n\n" + _active_shared_knowledge
            else:
                contexts[oasis_id] = _active_shared_knowledge

    return contexts


def _build_interpolated_context(
    enriched: Any | None,
) -> str:
    """LLM補間情報をエージェント共有コンテキストとして構築する.

    ScenarioAnalyzerが推定した市場情報（競合、収益モデル、市場規模等）を
    テキスト化してエージェントのプロンプトに注入する。
    """
    if not enriched or not hasattr(enriched, "interpolated_info"):
        return ""
    info = enriched.interpolated_info
    if not info:
        return ""

    lines: list[str] = []
    if info.competitors:
        lines.append(f"  主要競合: {', '.join(info.competitors[:5])}")
    if info.revenue_model:
        lines.append(f"  収益モデル: {info.revenue_model}")
    if info.target_users:
        lines.append(f"  ターゲット: {info.target_users}")
    if info.market_size_estimate:
        lines.append(f"  推定市場規模: {info.market_size_estimate}")
    if info.price_range:
        lines.append(f"  価格帯: {info.price_range}")
    if info.tech_stack:
        lines.append(f"  技術スタック: {info.tech_stack}")
    if info.team_size_estimate:
        lines.append(f"  推定チーム規模: {info.team_size_estimate}")
