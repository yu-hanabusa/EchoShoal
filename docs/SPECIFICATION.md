...
| Testing | pytest + pytest-asyncio + httpx | 355テスト |
...
num_rounds: int            # シミュレーション期間（1〜36ヶ月、デフォルト12）
...
| メソッド | パス | 説明 |
|---------|------|------|
| POST | `/api/simulations/research` | 市場調査実行（Google Trends/GitHub/Yahoo Finance） |
...
| POST | `/api/evaluation/run/{benchmark_id}` | ベンチマーク単発実行 |
| POST | `/api/evaluation/run/{benchmark_id}/multi` | ベンチマーク統計実行（N回） |
| POST | `/api/evaluation/run/{benchmark_id}/full` | 市場調査+シミュレーション+評価 |
...
| `/simulation/:jobId` | SimulationPage | 結果表示（関係図/SNSフィード/チャート） |
| `/simulation/:jobId/report` | ReportPage | レポート＋成功スコア |
| `/benchmarks` | BenchmarkListPage | ベンチマーク一覧 |
| `/benchmark/:jobId` | BenchmarkResultPage | ベンチマーク結果表示 |
...
- エージェント詳細: パーソナリティレーダーチャート（モーダル）

### 11.4 ReportPage

- 成功スコアゲージ（ScoreGauge: 0-100、色分け: 緑≥70/黄≥40/赤<40）
- スティッキーセクションナビ（SectionNav）
- エグゼクティブサマリー
- 8次元レーダーチャート（DimensionRadar）
- ディメンション別ミニトレンド（DimensionSparkline）
- リスク/機会カード（RiskOpportunityCard）
- 予測ハイライト
- レポートセクション（6セクション）
...
ECHOSHOAL_DEFAULT_ROUNDS=12
ECHOSHOAL_AGENT_ACTIVATION_RATE=0.7
...
ECHOSHOAL_OASIS_MESSAGE_WINDOW_SIZE=20 # メッセージ窓サイズ
ECHOSHOAL_OASIS_CONTEXT_TOKEN_LIMIT=16384
...
│       │   ├── ScoreGauge.tsx     # 成功スコアゲージ
│       │   ├── DimensionRadar.tsx  # 8次元レーダーチャート
│       │   ├── DimensionSparkline.tsx # ミニトレンドチャート
│       │   ├── RiskOpportunityCard.tsx # リスク/機会カード
│       │   └── SectionNav.tsx     # セクションナビゲーション
│       └── App.tsx              # ルーティング
