...
| NLP | GiNZA (spaCy) + SudachiPy | 日本語エンティティ抽出 |
...
| ディメンション | 値 | 説明 | 初期値 |
|---------------|-----|------|--------|
| ユーザー獲得率 | `user_adoption` | サービスのユーザー浸透度 | 0.3 |
| 収益ポテンシャル | `revenue_potential` | 収益化の可能性 | 0.3 |
| 技術成熟度 | `tech_maturity` | 技術的完成度 | 0.3 |
| 競合圧力 | `competitive_pressure` | 競合からの脅威度 | 0.3 |
| 規制リスク | `regulatory_risk` | 規制による制約 | 0.3 |
| 市場認知度 | `market_awareness` | 市場での認知度 | 0.3 |
| エコシステム健全性 | `ecosystem_health` | 連携サービス・コミュニティの活性度 | 0.3 |
| 資金調達環境 | `funding_climate` | 投資環境の良さ | 0.3 |
...
| 指標 | 説明 | 初期値 |
|------|------|--------|
| `economic_sentiment` | 経済センチメント | 0.5 |
| `tech_hype_level` | 技術ハイプレベル | 0.5 |
| `regulatory_pressure` | 規制圧力 | 0.3 |
| `remote_work_adoption` | リモートワーク普及率 | 0.45 |
| `ai_disruption_level` | AI破壊的変化の度合い | 0.3 |
...
入力: ScenarioInput + 文書（任意） + GitHub URL（任意）
  ...
  │    └─ 失敗時: デフォルト8体にフォールバック
  │
  ├─ 4. イベントスケジュール生成（EventScheduler）
  │    ├─ LLMが3〜5個のイベントを生成
  │    └─ 失敗時: パラメータベースの静的イベント
  │
  └─ 5. シミュレーション実行（SimulationEngine）
       │
       └─ ラウンド1〜N を繰り返し:
            ├─ グラフからディメンション同期（30%ブレンド）
...
各アクションは事前定義された**ディメンション影響マッピング**に基づいて市場に影響を与える。

**影響計算式:**
```
scaled_delta = base_delta × reputation_multiplier
reputation_multiplier = 0.5 + agent.reputation  # 範囲: 0.5〜1.5
new_value = clamp(current_value + scaled_delta, 0.0, 1.0)
```

**主要アクションの影響（抜粋）:**

| アクション | user_adoption | revenue_potential | competitive_pressure | その他 |
|-----------|:---:|:---:|:---:|------|
| adopt_service | +0.02 | +0.01 | | |
| build_competitor | | | +0.03 | tech_maturity +0.01 |
| launch_competing_feature | | | +0.04 | market_awareness +0.02 |
| subsidize | +0.01 | | | funding_climate +0.02 |
| invest_series | | +0.015 | | funding_climate +0.03 |
| open_source | | | | ecosystem_health +0.02, market_awareness +0.01 |
| regulate | | | | regulatory_risk +0.03 |
| price_undercut | | -0.02 | +0.03 | |
...
| ID | サービス | 年 | 主要期待トレンド |
|----|---------|-----|-----------------|
| `slack_2014` | Slack | 2014 | user_adoption +30%, competitive_pressure +20% |
| `notion_vs_confluence_2020` | Notion | 2020 | user_adoption +20%, revenue_potential +15% |
| `github_copilot_2022` | GitHub Copilot | 2022 | tech_maturity +25%, competitive_pressure +25% |
| `zoom_2020` | Zoom | 2020 | user_adoption +40%, revenue_potential +30% |
| `chatgpt_2022` | ChatGPT | 2022 | user_adoption +40%, funding_climate +25% |

### 8.2 評価スコア算出

```
direction_accuracy = 方向正解率（重み付き）     # 60%
magnitude_score = 1 - 平均規模誤差              # 30%
correlation = ピアソン相関（期待vs実際）         # 10%
overall_score = 0.6×direction + 0.3×magnitude + 0.1×correlation
```
...
**結果タブ:**
- ヒーローKPI: ユーザー獲得率
- 統計: 期間、エージェント数、LLM判断回数
- チャート2枚: 成長系ディメンション / 圧力系ディメンション
- エージェントテーブル: 最終状態 + パーソナリティレーダーチャート
- 行動タイムライン: ラウンドナラティブ + アクション一覧

**文書タブ:**
- アップロード済み文書の一覧テーブル

**グラフタブ:**
- Neo4j知識グラフのSVG可視化（ノード: Agent/Skill/Document/Company/Policy）
