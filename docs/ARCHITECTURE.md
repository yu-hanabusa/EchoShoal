...
OASISSimulationEngine
  ...
  ├─ 自動的な関係グラフ成長
  ├─ SQLiteにインタラクション記録
  └─ Neo4jへ同期（シミュレーション完了後）
```

### 5.1 エージェントコンテキスト構築

シミュレーション中、各エージェントには以下の情報が注入される。

```
全エージェント共通（シミュレーション開始時に1回取得）:
├─ 市場指標（8ディメンション値）
├─ 文書知識（GraphRAG → Neo4j文書ノードから取得）
├─ 統計データ（GraphRAG → e-Stat StatRecordから取得）
└─ LLM補間情報（競合・収益モデル・市場規模等）

エージェント固有（毎ラウンド構築、可視性フィルタ適用）:
├─ 自分の行動履歴（private含む全件）
└─ 他者の行動（public + 同種stakeholderのpartialのみ）
```

**設計上の注意:** GraphRAGRetrieverは8種のコンテキスト取得機能を持つが、
OASIS実行中は行動履歴やエージェント関係がNeo4jに未記録のため、
可視性フィルタリングはインメモリで実行し、Neo4jからは静的知識
（文書・統計）のみを取得するハイブリッド方式を採用している。
シミュレーション完了後にOASIS SQLite → Neo4jへ同期されることで、
フルGraphRAGパスがレポート生成や後続分析で利用可能になる。
